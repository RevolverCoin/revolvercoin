...
###   This script attempts to download the signature file SHA256SUMS.asc from revolvercoin.org
...
WORKINGDIR="/tmp/revolvercoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://revolvercoin.org/bin/"
VERSIONPREFIX="revolvercoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'revolvercoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the RevolverCoin Core binary release signing key installed?"
   fi
